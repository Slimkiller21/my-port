  ...
  let consoles = document.getElementById('console')
    ...
    const dataRef = ref(db,'SETOR_DE_HOMENS/')
    ...
    function writeUserData(nome, idade, funcao,) {
        ...
        set(ref(db, "SETOR_DE_HOMENS/" +" " + nome_), {
                Nome_completo: nome_,
                Idade : idade_,
                função : funcao_,
        })
...
// Create a reference to the "SETOR_DE_HOMENS" node in the database
const evangelizacaoRef = ref(db, 'SETOR_DE_HOMENS/');
// Retrieve the data once from the "SETOR_DE_HOMENS" node
get(evangelizacaoRef)
  .then((snapshot) => {
    // Iterate over each child node of "SETOR_DE_HOMENS"
...
const userRef = ref(db, `SETOR_DE_HOMENS/${key}`);
const userData = {
  Nome_completo: nome,
  Idade: data,
  função: funcao,
