    ...
    import { getDatabase, ref,update,set,get, onValue} from "https://www.gstatic.com/firebasejs/9.17.2/firebase-database.js";
  ...
  let consoles = document.getElementById('console')
    ...
    const dataRef = ref(db, "SETOR DE MÚSICA/");
    ...
    function writeUserData(nome, idade, funcao,) {
        ...
        set(ref(db, "SETOR DE MÚSICA/" +" " + nome_), {
                Nome_completo: nome_,
                Idade : idade_,
                função : funcao_,
        })
...
// Create a reference to the "SETOR DE MÚSICA" node in the database
const evangelizacaoRef = ref(db, 'SETOR DE MÚSICA/');
// Retrieve the data once from the "SETOR DE MÚSICA" node
get(evangelizacaoRef)
  .then((snapshot) => {
    // Iterate over each child node of "SETOR DE MÚSICA"
...
const userRef = ref(db, `SETOR DE MÚSICA/${key}`);
const userData = {
  Nome_completo: nome,
  Idade: data,
  função: funcao,
...
$(window).on('load',function(){
	setTimeout(function(){
    console.log('wawa') // allowing 3 secs to fade out loader
