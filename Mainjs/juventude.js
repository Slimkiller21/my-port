  ...
  let consoles = document.getElementById('console')
    ...
    const dataRef = ref(db, 'JUVENTUDE/')
...
// Create a reference to the "JUVENTUDE" node in the database
const evangelizacaoRef = ref(db, 'JUVENTUDE/');
// Retrieve the data once from the "JUVENTUDE" node
get(evangelizacaoRef)
  .then((snapshot) => {
    // Iterate over each child node of "JUVENTUDE"
...
const userRef = ref(db, `JUVENTUDE/${key}`);
const userData = {
  Nome_completo: nome,
  Idade: data,
  função: funcao,
