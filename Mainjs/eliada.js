...
// Create a reference to the "ELIADA/" node in the database
const evangelizacaoRef = ref(db, 'ELIADA//');
// Retrieve the data once from the "ELIADA/" node
get(evangelizacaoRef)
  .then((snapshot) => {
    // Iterate over each child node of "ELIADA/"
...
const userRef = ref(db, `ELIADA/${key}`);
const userData = {
  Nome_completo: nome,
  Idade: data,
  função: funcao,
